		...
		<div className='grid w-full grid-cols-3 gap-4 rounded-md p-4 shadow-md dark:shadow-sm dark:shadow-white max-md:grid-cols-1'>
			<div className='col-span-2 flex items-center gap-2'>
				...
				<div className='flex flex-col'>
					<h1 className='font-space_grotesk text-lg font-bold'>{cart.title}</h1>
					<p className='line-clamp-2 text-sm text-muted-foreground'>
						{cart.description}
					</p>
					<h1 className='font-space_grotesk font-bold md:hidden'>
						{(cart.currentPrice * cart.quantity).toLocaleString('en-US', {
							style: 'currency',
							currency: 'USD',
						})}
					</h1>
			...
			<div className='flex items-center justify-end gap-2'>
				...
				<h1 className='font-space_grotesk text-xl font-bold max-md:hidden'>
